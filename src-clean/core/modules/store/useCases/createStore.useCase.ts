...
export class CreateStoreUseCase {
  ...
  async execute(dto: CreateStoreInputDTO): Promise<CoreResponse<Store>> {
    ...
    const [createErr, store] = Store.create({
      name: dto.name,
    ...
    if (createErr) return [createErr, undefined];

    const [findErr, existingStoreEmail] =
      await this.storeGateway.findStoreByEmail(dto.email);
    if (findErr) return [findErr, undefined];
    if (existingStoreEmail) {
      return [
        new ResourceConflictException('Store with this email already exists'),
        undefined,
      ];
    }

    const [findErrCnpj, existingStoreCnpj] =
      await this.storeGateway.findStoreByCnpj(dto.cnpj);
    if (findErrCnpj) return [findErrCnpj, undefined];
    if (existingStoreCnpj) {
      return [
        new ResourceConflictException('Store with this CNPJ already exists'),
        undefined,
      ];
    }

    const [findErrName, existingStoreName] =
      await this.storeGateway.findStoreByName(dto.name);
    if (findErrName) return [findErrName, undefined];
    if (existingStoreName) {
      return [
        new ResourceConflictException('Store with this name already exists'),
        undefined,
      ];
    }

    const [saveErr] = await this.storeGateway.saveStore(store);
    if (saveErr) return [saveErr, undefined];
